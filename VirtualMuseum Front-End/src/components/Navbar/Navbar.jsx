...
const PharaohNavbar = () => {
  ...
  // تتبع السكرول وساعة القاهرة اللايف
  useEffect(() => {
    // Scroll Logic
    ...
    const updateTime = () => {
      ...
      // المتحف بيفتح من 9 الصبح لـ 5 المغرب
    ...
    <>
      ...
      >
        ...
        <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-4 px-6 relative z-10">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <li key={link.name} className="group/nav relative py-6">
                  ...
                  {/* ================= الـ Mega Menu (تظهر فقط عند القرب من Categories) ================= */}
                  {link.hasMegaMenu && (
                    <div className="absolute top-[100%] left-1/2 -translate-x-1/2 w-[800px] bg-[#0a0a0f]/95 backdrop-blur-3xl border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.8)] rounded-3xl opacity-0 invisible group-hover/nav:opacity-100 group-hover/nav:visible transition-all duration-500 transform translate-y-4 group-hover/nav:translate-y-0 p-8 grid grid-cols-3 gap-6 pointer-events-none group-hover/nav:pointer-events-auto">
                      ...
                      <Link href="/Categories?filter=Statues" className="group/card relative overflow-hidden rounded-2xl h-48 cursor-pointer">
                        ...
                        <Image src="/assets/images/ramses.jpg" alt="Statues" layout="fill" objectFit="cover" className="group-hover/card:scale-110 transition-transform duration-700" />
                      ...
                      {/* كارت 2 في الميجا منيو */}
                      <Link href="/Categories?filter=Jewelry" className="group/card relative overflow-hidden rounded-2xl h-48 cursor-pointer">
                        <div className="absolute inset-0 bg-black/40 group-hover/card:bg-black/20 transition-colors z-10"></div>
                        <Image src="/assets/images/mask.jpg" alt="Jewelry" layout="fill" objectFit="cover" className="group-hover/card:scale-110 transition-transform duration-700" />
                        <div className="absolute bottom-0 left-0 w-full p-4 z-20 bg-gradient-to-t from-black to-transparent">
                      ...
                      {/* كارت 3 في الميجا منيو */}
                      <Link href="/Categories?filter=Papyri" className="group/card relative overflow-hidden rounded-2xl h-48 cursor-pointer">
                        <div className="absolute inset-0 bg-black/40 group-hover/card:bg-black/20 transition-colors z-10"></div>
                        <Image src="/assets/images/papyrus.jpg" alt="Papyri" layout="fill" objectFit="cover" className="group-hover/card:scale-110 transition-transform duration-700" />
                        <div className="absolute bottom-0 left-0 w-full p-4 z-20 bg-gradient-to-t from-black to-transparent">
          ...
          {/* ================= 3. Search & Magnetic Actions ================= */}
          <div className="flex items-center gap-4 lg:gap-6">
            
            <div className="hidden sm:flex items-center gap-2 text-gray-400 border-r border-white/10 pr-6">
              {/* أيقونات مغناطيسية */}
              <Magnetic>
                <div className="relative p-2 cursor-pointer hover:text-[#D4AF37] transition-colors group">
                  <Heart size={20} />
                  <span className="absolute top-0 right-0 bg-red-600 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold border border-[#050505] shadow-lg">0</span>
                </div>
              </Magnetic>
              ...
              <Magnetic>
                <div className="relative p-2 cursor-pointer hover:text-[#D4AF37] transition-colors">
                  <Bookmark size={20} />
                  <span className="absolute top-0 right-0 bg-[#D4AF37] text-black text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold border border-[#050505] shadow-lg">0</span>
                </div>
              </Magnetic>
            </div>
            ...
            <div className="flex items-center gap-6">
              ...
              {/* زرار الحجز المغناطيسي (Magnetic CTA) */}
              <Magnetic>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            {/* زر الإغلاق */}
            <button 
              className="absolute top-8 right-8 text-gray-400 hover:text-[#D4AF37] transition-colors p-2"
            ...
            <div className="flex flex-col items-center gap-8 mt-10">
                ...
                return (
                  ...
                  >
                    <Link 
                      href={link.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`text-3xl font-serif font-black uppercase tracking-widest transition-colors flex items-center gap-3 ${
              ...
              {/* ================= أزرار الموبايل (تمت إضافة زر الحجز هنا) ================= */}
              <motion.div
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: navLinks.length * 0.1 }}
                className="mt-8 flex flex-col items-center gap-6 w-full px-8"
              >
                {/* زرار الحجز (Book Ticket) البارز في الموبايل */}
                <Link href="/Booking" onClick={() => setIsMobileMenuOpen(false)} className="w-full text-center bg-gradient-to-r from-[#D4AF37] to-[#B8962E] text-black py-4 rounded-full font-black uppercase tracking-widest shadow-[0_0_20px_rgba(212,175,55,0.4)] hover:shadow-[0_0_30px_rgba(212,175,55,0.7)] transition-shadow">
                  Book Your Tickets
                </Link>
                
                {/* أيقونات الحفظ */}
                <div className="flex gap-8 text-[#D4AF37] mt-2">
                  <div className="flex flex-col items-center gap-1 cursor-pointer"><Heart size={24} /><span className="text-[10px] uppercase">Saved</span></div>
                  <div className="flex flex-col items-center gap-1 cursor-pointer"><Bookmark size={24} /><span className="text-[10px] uppercase">Bookmarks</span></div>
                </div>
